...
//{
//    public class SkyboxFunctionality
//    {
//        SkyboxGameobjectsPool skyboxElements;

//        public SkyboxFunctionality()
//        {
//            skyboxElements = new SkyboxGameobjectsPool();
//            // No need to pass skybox configuration
//            skyboxElements.Initialize3DObjects(new SkyboxConfiguration());
//        }

//        public void ApplyOnMaterial(SkyboxConfiguration config, Material selectedMat, float dayTime, float normalizedDayTime, int slotCount = 5, Light directionalLightGO = null, float cycleTime = 24)
//        {
//            float percentage = normalizedDayTime * 100;

//            // General Values
//            selectedMat.SetColor(SkyboxShaderUtils.LightTint, config.directionalLightLayer.tintColor.Evaluate(normalizedDayTime));
//            selectedMat.SetVector(SkyboxShaderUtils.LightDirection, directionalLightGO.transform.rotation.eulerAngles);

//            // Apply Base Values
//            selectedMat.SetColor(SkyboxShaderUtils.SkyColor, config.skyColor.Evaluate(normalizedDayTime));
//            selectedMat.SetColor(SkyboxShaderUtils.GroundColor, config.groundColor.Evaluate(normalizedDayTime));

//            // Apply Horizon Values
//            selectedMat.SetColor(SkyboxShaderUtils.HorizonColor, config.horizonColor.Evaluate(normalizedDayTime));
//            selectedMat.SetFloat(SkyboxShaderUtils.HorizonHeight, GetTransitionValue(config.horizonHeight, percentage, 0f));
//            selectedMat.SetFloat(SkyboxShaderUtils.HorizonWidth, GetTransitionValue(config.horizonWidth, percentage, 0f));
//            selectedMat.SetTexture(SkyboxShaderUtils.HorizonMask, config.horizonMask);
//            selectedMat.SetVector(SkyboxShaderUtils.HorizonMaskValues, config.horizonMaskValues);
//            selectedMat.SetColor(SkyboxShaderUtils.HorizonPlaneColor, config.horizonPlaneColor.Evaluate(normalizedDayTime));
//            selectedMat.SetFloat(SkyboxShaderUtils.HorizonPlaneHeight, GetTransitionValue(config.horizonPlaneHeight, percentage, -1f));
...
//                RenderSettings.ambientSkyColor = config.ambientSkyColor.Evaluate(normalizedDayTime);
//                RenderSettings.ambientEquatorColor = config.ambientEquatorColor.Evaluate(normalizedDayTime);
//                RenderSettings.ambientGroundColor = config.ambientGroundColor.Evaluate(normalizedDayTime);
//            }
//            else
//            {
...
//            if (config.useFog)
//            {
//                RenderSettings.fogColor = config.fogColor.Evaluate(normalizedDayTime);
...
//                directionalLightGO.color = config.directionalLightLayer.lightColor.Evaluate(normalizedDayTime);
...
//        public void ApplyOnSatelliteLayer(SkyboxConfiguration config, float dayTime)
//        {
//            List<SatelliteReferences> satelliteRefs = skyboxElements.GetSatelliteAllActiveSatelliteRefs(config.satelliteLayers);
